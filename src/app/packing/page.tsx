...
export default function PackingPage() {
  ...
  useEffect(() => { if (!activeTrip) router.push('/home'); }, [activeTrip]);
  ...
  // Escape key closes smart-save modal
  useEffect(() => {
    if (!showSSModal) return;
    const handler = (e: KeyboardEvent) => { if (e.key === 'Escape') setUI({ ssItem: null, ssTabKey: null, ssTarget: null }); };
              ...
              {cat.items.map((item, ii) => (
                <div key={ii} className="flex items-center gap-2 py-2 border-b border-[#f5f5f5] last:border-0">
                  <div
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && toggleDone(activeKey, ci, ii)}
                    className="w-[44px] h-[44px] flex items-center justify-center shrink-0 cursor-pointer"
                  >
                    <div className={`w-[21px] h-[21px] border-2 rounded-[6px] flex items-center justify-center transition-all text-xs ${
                  ...
                  <span
                    ...
                    className="text-[#ddd] text-[15px] cursor-pointer p-[10px] hover:text-[#ff6b6b] transition-colors shrink-0"
                  >🗑️</span>
                </div>
              ))}
      ...
      {showSSModal && (
        <div className="fixed inset-0 bg-black/50 z-[200] flex items-end justify-center pb-5 px-4">
          <div
            role="dialog"
            aria-modal="true"
            className="bg-white rounded-[22px] p-6 max-w-[480px] w-full"
            onKeyDown={(e) => {
              if (e.key !== 'Tab') return;
          ...
          >
            ...
            <div className="text-[13px] text-[#6b6b6b] text-center mb-4 leading-relaxed">הוספת פריט חדש לרשימה.<br />רוצה לשמור אותו לטיולים הבאים?</div>
            ...
            <div className="flex flex-col gap-2 mb-3">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-[#6b6b6b] mt-0.5">יופיע בטיולים הבאים</div>
                    </div>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-[#6b6b6b] mt-0.5">לכל בני המשפחה</div>
                </div>
                {ui.ssTarget === 'shared' && <span className="text-[#667eea] text-[15px]">✓</span>}
              </div>
            </div>

            <button autoFocus onClick={() => { confirmSS(); }} className="w-full py-3 bg-gradient-to-br from-[#667eea] to-[#764ba2] text-white border-none rounded-[13px] text-[15px] font-bold cursor-pointer mb-2">שמור 💾</button>
            <button onClick={() => setUI({ ssItem: null, ssTabKey: null, ssTarget: null })} className="w-full py-2.5 bg-transparent border-2 border-[#e8e4ff] text-[#6b6b6b] rounded-[13px] text-[13px] font-semibold cursor-pointer">לא תודה, רק לטיול הזה</button>
          </div>
        </div>
      )}
    </div>
