...
export default function DonePage() {
  ...
  function handleBackToPacking() {
    // Just nav back without abandoning
    ...
    <div className="min-h-[100dvh] flex items-start justify-center p-5">
      <div className="bg-white rounded-3xl p-7 max-w-[480px] w-full shadow-[0_20px_60px_rgba(0,0,0,0.2)] my-auto">
        ...
        <div className="text-center pt-1.5 pb-5">
          ...
          <div className="text-sm text-[#888] leading-relaxed">ארזת {done} מתוך {total} פריטים.</div>
        </div>
        ...
        <div className="bg-[#f9f7ff] rounded-2xl p-4 mb-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-[#888] mt-0.5">{dur} ימים{wLabel ? ' • ' + wLabel : ''}{aStr ? ' • ' + aStr : ''}</div>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-1.5">
            <div className="text-center bg-white rounded-[11px] py-2.5 px-1">
              ...
              <div className="text-[11px] text-[#aaa] mt-0.5 font-semibold">פריטים ארוזים</div>
            </div>
            <div className="text-center bg-white rounded-[11px] py-2.5 px-1">
              <div className="text-xl font-extrabold text-[#667eea]">{travelers.length}</div>
              <div className="text-[11px] text-[#aaa] mt-0.5 font-semibold">נוסעים</div>
            </div>
            <div className="text-center bg-white rounded-[11px] py-2.5 px-1">
              <div className="text-xl font-extrabold text-[#667eea]">{dur}</div>
              <div className="text-[11px] text-[#aaa] mt-0.5 font-semibold">ימים</div>
            </div>
          </div>
        </div>
          ...
          {travelers.map(id => {
            ...
            return (
              <div key={id} className="flex flex-col items-center gap-0">
                ...
                <div className="text-[11px] text-[#888] font-semibold mt-0.5">{p.name}</div>
              </div>
            );
          })}
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <button onClick={handleBackToPacking} className="w-full py-3 bg-transparent text-[#aaa] border-2 border-[#f0f0f0] rounded-[14px] text-sm font-semibold cursor-pointer">
            חזור לרשימה
          </button>
        </div>
      </div>
